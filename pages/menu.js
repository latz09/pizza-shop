		  ...
		  <div className="pb-24 mt-8">
			<div className="">
			  <AnimatePresence mode='wait'>
				{menuCategory === 1 && (
				  <motion.div key="byo" initial="hidden" animate="visible" exit="hidden" variants={fadeIn}>
					<BuildYourOwn />
				  </motion.div>
				)}
				{menuCategory === 2 && (
				  <motion.div key="pf" initial="hidden" animate="visible" exit="hidden" variants={fadeIn}>
					<PizzaFries />
				  </motion.div>
				)}
				{menuCategory === 3 && (
				  <motion.div key="as" initial="hidden" animate="visible" exit="hidden" variants={fadeIn}>
					<AllSpecials />
				  </motion.div>
				)}
				{menuCategory === 4 && (
				  <motion.div key="sw" initial="hidden" animate="visible" exit="hidden" variants={fadeIn}>
					<Sandwiches />
				  </motion.div>
				)}
			  </AnimatePresence>
